...
export async function gerarReprovadosTafPdfBlobWeb(
  ...
  const cols: Col[] = [
    ...
    { title: 'Modalidades reprovadas', w: usableW * 0.44, get: modalidadesTexto },
  ];
  ...
  const drawCabecalho = (pageIndex: number) => {
    ...
    doc.text(pdfTexto('Militares reprovados no TAF'), marginX, y);
    y += 16;
    ...
    doc.text(
      pdfTexto(
        `Reprovados em pelo menos um teste · ${itens.length} militares · Gerado em ${geradoEm}`,
      ),
      marginX,
      y,
    );
