...
/** PDF A4 paisagem de concluídos — sem assinatura de aplicador. */
export async function gerarConcluidosTafPdfBlobWeb(itens: ConcluidoTafItem[]): Promise<Blob> {
  ...
  type Col = { title: string; w: number; get: (r: ConcluidoTafItem) => string };
  const cols: Col[] = [
    { title: 'NIP', w: usableW * 0.14, get: (r) => r.nip || '—' },
    { title: 'Nome', w: usableW * 0.34, get: (r) => r.nome || '—' },
    { title: 'P/G', w: usableW * 0.1, get: (r) => r.postoGrad || '—' },
    { title: 'Categoria', w: usableW * 0.14, get: (r) => r.categoria || '—' },
    { title: 'Situacao', w: usableW * 0.12, get: () => 'Concluido' },
    { title: 'Modalidades', w: usableW * 0.16, get: () => 'C:OK N:OK P:OK' },
  ];
  ...
  const drawCabecalho = (pageIndex: number) => {
    ...
    doc.text(pdfTexto('Militares com TAF concluido'), marginX, y);
    y += 16;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.setTextColor(107, 114, 128);
    doc.text(
      pdfTexto(`Todas as modalidades concluidas · ${itens.length} militares · Gerado em ${geradoEm}`),
      marginX,
      y,
    );
    if (pageIndex > 0) {
