...
export type ResumoAplicacaoPdfBloco = {
 ...
 * Um único bloco = mesmo layout do "Salvar" pós-rúbrica do aplicador.
 */
export async function gerarResumoAplicacaoPdfBlobWeb(
  resultados: ResultadoCorridaItem[],
  aplicadorAssinatura?: AplicadorAssinaturaResumo,
): Promise<Blob> {
  return gerarResumosAplicacaoPdfBlobWeb([{ resultados, aplicadorAssinatura }]);
}

/**
 * Vários resumos (ex.: todas as sessões do dia) num único PDF —
 * cada bloco começa em folha nova, no mesmo layout do Salvar.
 */
export async function gerarResumosAplicacaoPdfBlobWeb(
  blocos: ResumoAplicacaoPdfBloco[],
): Promise<Blob> {
  const validos = blocos.filter((b) => b.resultados.length > 0);
  if (validos.length === 0) {
