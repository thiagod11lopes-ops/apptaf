...
export function ConfirmacaoExcluirRestritoModal({
  visible,
  nip,
  nome,
  loading = false,
    ...
    >
      <View style={styles.bodyInner}>
        ...
        <Text style={[styles.message, { color: theme.text }]}>
          A dispensa de <Text style={styles.strong}>{nomeExibir}</Text>
          {nipFmt ? (
            <>
              {' '}
          ...
          será removida. O militar voltará a constar nas contagens de pendente/parcial/concluído
          conforme seus testes.
        </Text>
...
const styles = StyleSheet.create({
  bodyInner: { gap: 14, alignItems: 'center' },
  warnBox: {
    width: 64,
    height: 64,
    borderRadius: 16,
  ...
  btnDangerText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' },
});
