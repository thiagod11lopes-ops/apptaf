...
type Props = {
  reserva: ReservaAgendamento | null;
  loading?: boolean;
  onClose: () => void;
  onConfirm: () => void;
};

function postoReserva(r: ReservaAgendamento): string {
  return ((r.posto || r.oficial || r.praca || '').trim() || '—').toUpperCase();
}

export function ConfirmacaoExcluirReservaAgendamentoModal({
  reserva,
  loading = false,
  onClose,
  onConfirm,
  ...
  const visible = !!reserva;
  const nipKey = reserva ? nipDigitos(reserva.nip) : '';
  const nipFmt = nipKey ? formatNipInput(nipKey) : reserva?.nip || '—';
  const nome =
    reserva
      ? nomeBareSemPosto(reserva.nome || '').trim() || (reserva.nome || '—').trim()
      : '';
      ...
      {reserva ? (
        <View style={styles.bodyInner}>
          ...
          <Text style={[styles.message, { color: theme.text }]}>
            Remover <Text style={styles.strong}>{postoReserva(reserva)} {nome}</Text>
            {'\n'}NIP <Text style={styles.strong}>{nipFmt}</Text>?
          </Text>
          <Text style={[styles.hint, { color: theme.textMuted }]}>
            A inscrição será cancelada e a vaga liberada na página pública. Esta ação não pode ser
            desfeita.
          </Text>
        </View>
      ) : null}
